...
export default class Egout extends Phaser.Scene{
	...
	preload() {
		...
		this.load.spritesheet('DropBossSprite', 'src/assets/DropBossSprite.png',{frameWidth: 34, frameHeight: 34});
		this.load.audio('BossMortSound', 'src/assets/bruit_bossMort.mp3');
	}
	...
	create() {
		...
		// Affichage du boss
		this.boss = new Boss(this, 1365, 608, 'SpriteBoss', this.player);
		...
		this.physics.add.collider(this.boss, collisionLayer);
		this.physics.add.collider(this.boss, eauLayer);
	...
	update() {
		...
		this.enemies.getChildren().forEach(zombie => {
			if (zombie.hasBeenHit && window.myGameValues.hpValues >= 0) {
				this.cameras.main.shake(50, 0.05);
				zombie.hasBeenHit = false;
			}
		});
		...
		this.boss?.update();
		if (this.boss.hasBeenHit && window.myGameValues.hpValues >= 0) {
			this.cameras.main.shake(50, 0.05);
