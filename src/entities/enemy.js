...
export default class Zombie1 extends Phaser.GameObjects.Sprite {
	...
	overlapEnemies(enemies) {
		...
		kill() {
			// Génére un nombre aléatoire entre 1 et 3
			...
			if (random === 1) {
				...
				this.scene.physics.add.overlap(drop, this.scene.player, () => {
					// Incrémente la variable de points de vie si le joueur a moins de 5 hp
					if (window.myGameValues.hpValues < 5) {
						// Incrémente la variable de points de vie
						window.myGameValues.hpValues += 1;
					}
					// Détruit le sprite de récupération de vie
					drop.destroy();
				});
			}
		...
		update() {
			...
			if (vecteurDeplacement.length() < ZOMBIE_RANGE) {
				...
				// Si le zombie est de type rapide, multiplie par 3 sa vitesse de deplacement
				if (this.type === "rapide") {
